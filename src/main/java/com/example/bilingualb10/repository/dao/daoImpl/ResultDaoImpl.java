...
public class ResultDaoImpl implements ResultDao {
    ...
    private final RowMapper<TestResultResponse> testResultResponseRowMapper = (resultSet, rowMapper) -> {
        ...
        Integer maxScore = resultSet.getInt("max_score");
        ...
        return new TestResultResponse(userFullName, dateOfSubmission, testName, checked, finalScore, maxScore, userId, testId, questionResultResponses);
    };
    ...
    public List<TestResultResponse> getAllTestResults() {
        String sql = """
                ...
                from results r
                        ...
                        full join questions q on a.question_id = q.id
                        full join users u on a.user_id = u.id
                        full join tests t on q.test_id = t.id
                group by t.title, concat(u.first_name,' ', u.last_name), r.checked, u.id, t.id 
                order by r.checked desc ;
                """;
        return jdbcTemplate.query(sql, ((rs, rowNum) -> {
            ...
            testResult.setUserId(rs.getLong(5));
            testResult.setTestId(rs.getLong(6));
    ...
    public TestResultResponse getTestResultById(Long testId, Long userId) {
        String sql = """
                       ...
                       concat(u.first_name,' ', u.last_name) as full_name,
                        (select max(r.date_of_submission) at time zone 'Asia/Bishkek') as date_of_submission,
                        r.checked,
                        t.title,
                        (select sum(r.score) as final_score),
                        (select 10 * count(r.score) as max_score),
                        t.id as test_id,
                        u.id as user_id
                        from results r
                ...
                join tests t on q.test_id = t.id
                    where t.id = ? and u.id = ?
                    group by t.title, concat(u.first_name,' ', u.last_name), r.checked, t.id, u.id
                       """;
            return jdbcTemplate.queryForObject(sql, testResultResponseRowMapper, testId, userId);
    }
